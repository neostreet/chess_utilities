...
static char usage[] =
"usage: count_piece_moves (-binary_format) (-i_am_white) (-i_am_black) [white | black] index filename\n";
...
int main(int argc,char **argv)
{
  ...
  sscanf(argv[curr_arg+1],"%d",&index);
  ...
  for ( ; ; ) {
    ...
    if (by_white)
      printf("%d %s\n",curr_game.white_pieces[index].move_count,filename);
    else
      printf("%d %s\n",curr_game.black_pieces[index].move_count,filename);
  }
