...
static char usage[] =
"usage: fprint_space_counts (-binary_format) (-i_am_white) (-i_am_black) (-my_count_first) (-diff_first)\n"
...
int main(int argc,char **argv)
{
  ...
  for ( ; ; ) {
    ...
    if (!bMaxDiff) {
      if (!bMyCountFirst) {
        first_count = seirawan_count[WHITE];
        second_count = seirawan_count[BLACK];
      }
      else {
        if (!curr_game.orientation) {
          ...
          first_count = seirawan_count[BLACK];
          second_count = seirawan_count[WHITE];
        }
      }
    }
    else {
